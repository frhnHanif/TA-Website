...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-xl shadow-sm p-6 text-white flex items-center justify-between">
            <div>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-sm p-6 text-white flex items-center justify-between">
            <div>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl shadow-sm p-6 text-white flex items-center justify-between">
            <div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div class="bg-gray-200 p-1 rounded-lg inline-flex w-full sm:w-auto shadow-inner">
            <button onclick="updateCharts('daily')" id="btn-daily" class="filter-btn flex-1 sm:flex-none px-6 py-2 text-sm font-bold rounded-md bg-white shadow-sm text-primary transition-all">Harian</button>
            <button onclick="updateCharts('weekly')" id="btn-weekly" class="filter-btn flex-1 sm:flex-none px-6 py-2 text-sm font-medium rounded-md text-gray-500 hover:text-gray-800 transition-all">Mingguan</button>
            <button onclick="updateCharts('monthly')" id="btn-monthly" class="filter-btn flex-1 sm:flex-none px-6 py-2 text-sm font-medium rounded-md text-gray-500 hover:text-gray-800 transition-all">Bulanan</button>
            <button onclick="updateCharts('yearly')" id="btn-yearly" class="filter-btn flex-1 sm:flex-none px-6 py-2 text-sm font-medium rounded-md text-gray-500 hover:text-gray-800 transition-all">Tahunan</button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 relative">
            <div id="loader-TempHum" class="absolute inset-0 bg-white bg-opacity-70 z-10 hidden flex justify-center items-center rounded-xl"><i class="fa-solid fa-spinner fa-spin text-3xl text-primary"></i></div>
        ...
        <!-- Grafik Gas Amonia -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 relative">
            <div id="loader-Ammonia" class="absolute inset-0 bg-white bg-opacity-70 z-10 hidden flex justify-center items-center rounded-xl"><i class="fa-solid fa-spinner fa-spin text-3xl text-primary"></i></div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 relative">
            <div id="loader-TotalMass" class="absolute inset-0 bg-white bg-opacity-70 z-10 hidden flex justify-center items-center rounded-xl"><i class="fa-solid fa-spinner fa-spin text-3xl text-primary"></i></div>
...
<script>
    ...
    chartTempHum = new ApexCharts(document.querySelector("#chartTempHum"), {
        ...
        yaxis: [
            { title: { text: 'Suhu (°C)', style: { color: '#f97316' } }, labels: { style: { colors: '#f97316' } } },
            { opposite: true, title: { text: 'Kelembaban (%)', style: { color: '#3b82f6' } }, labels: { style: { colors: '#3b82f6' } } }
        ],
        legend: { position: 'top' }
    });
    chartTempHum.render();
    ...
    chartTotalMass = new ApexCharts(document.querySelector("#chartTotalMass"), {
        ...
        dataLabels: { enabled: false },
        yaxis: { labels: { formatter: (value) => { return value.toFixed(1) + " kg" } } }
    });
    ...
    function updateCharts(period) {
        ...
        const textDesc = period === 'daily' ? 'Pergerakan rata-rata per jam (24 jam terakhir)' : (period === 'weekly' ? 'Pergerakan rata-rata per hari (7 hari terakhir)' : 'Pergerakan rata-rata per hari (30 hari terakhir)');
